...
class HomeFragment : Fragment() {
    ...
    private fun loadSellingList() {
        ...
        sellingProcessAdapter.setOnClickListener { position, _ ->
            articleSellList.removeAt(position).apply {
                val list = articleSellList.map { SellingProcessDtoModel(it.name, it.price, it.quantity) }
                sellingProcessAdapter.updateItems(list)
