...
class DatabaseHelper(private val context: Context) :
    ...
    fun getTotalList(type: String): List<ReportModel> {
        val db = readableDatabase
        val reportList = ArrayList<ReportModel>()
        val cursor = db.rawQuery("SELECT COUNT(*) FROM selling_process", null)
        val cursor2 = db.rawQuery("SELECT SUM(Quantity) FROM selling_process", null)
        val cursor3 = db.rawQuery("SELECT SUM(Amount) FROM selling_process", null)

        var customerCount = 0
        var productCount = 0
        var amount = 0
        if (cursor.moveToFirst()) {
           customerCount = cursor.getInt(0)?: 0
        }
        if (cursor2.moveToFirst()) {
            productCount = cursor2.getInt(0)?: 0
        }
        if (cursor3.moveToFirst()) {
            amount = cursor3.getInt(0)?: 0
        }

        val totalList = ReportModel(
            customerCount,
            productCount,
            amount,
            type
        )
        reportList.add(totalList)
        return reportList
    }
    fun getTotalListFromPaymentType(type:String, userID: Int): ReportModel {
        val db = readableDatabase
        val reportList = ArrayList<ReportModel>()
        val cursor = db.rawQuery("SELECT COUNT(*) FROM selling_process WHERE Type = '$type' AND UserID = '$userID'", null)
        val cursor2 = db.rawQuery("SELECT SUM(Quantity) FROM selling_process WHERE Type = '$type' AND UserID = '$userID'", null)
        val cursor3 = db.rawQuery("SELECT SUM(Amount) FROM selling_process WHERE Type = '$type' AND UserID = '$userID'", null)
    ...
    fun getTotalListFromPaymentTypeMulti(type:String): ReportModel {
        val db = readableDatabase
        val reportList = ArrayList<ReportModel>()
        val cursor = db.rawQuery("SELECT COUNT(*) FROM selling_process WHERE Type = '$type'", null)
        val cursor2 = db.rawQuery("SELECT SUM(Quantity) FROM selling_process WHERE Type = '$type'", null)
        val cursor3 = db.rawQuery("SELECT SUM(Amount) FROM selling_process WHERE Type = '$type'", null)
    ...
    fun getSellingProcess(): MutableList<ZReportModel> {
        val db = readableDatabase
        val sellingList = ArrayList<ZReportModel>()
        val cursor = db.rawQuery("SELECT * FROM selling_process", null)
        val productNumber = cursor.getColumnIndex("ProductNumber")
        ...
        val userID = cursor.getColumnIndex("UserID")
        val type = cursor.getColumnIndex("Type")

        if (productNumber == -1 || quantityIndex == -1 || priceIndex == -1 || amount == -1 || userID == -1 || type == -1) {
            cursor.close()
        ...
        if (cursor.moveToFirst()) {
            do {
                val selling = ZReportModel(
                    cursor.getString(type),
                    cursor.getInt(productNumber),
